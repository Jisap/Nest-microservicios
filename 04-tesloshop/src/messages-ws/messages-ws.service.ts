...
export class MessagesWsService {
  ...
  async registerClient(client: Socket, ) {                 // Detectado un cliente que se conecta en el gateway, lo recibe esta función

    // const user = await this.userRepository.findOneBy({ id: userId })     // Verificamos que el usuario del token está en bd
    // if (!user) throw new Error('User not found')                         // Si no existe el usuario mensaje de error
